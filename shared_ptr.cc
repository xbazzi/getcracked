...
template <class T, class Deleter = default_deleter<T>> struct control_block : private Deleter
{
...
template <class T, class Deleter = default_deleter<T>> class shared_ptr
{
  private:
    using Pointer_T = T *;
  ...
  public:
    shared_ptr() : p_obj{nullptr}, p_cb{nullptr}
    {
    }

    template <class U = Deleter>
        requires std::default_initializable<U>
    shared_ptr(Pointer_T pointer) : p_obj{pointer}
    {
    ...
    shared_ptr(Pointer_T pointer, const Deleter &d) : p_obj{pointer}
    {
        p_cb = static_cast<Pointer_CB>(::operator new(sizeof(CB)));
    ...
    shared_ptr(const shared_ptr &other) noexcept : p_obj{other.p_obj}, p_cb{other.p_cb}
    {
        if (p_cb)
        {
            p_cb->add_ref();
    ...
    shared_ptr(shared_ptr &&other) noexcept : p_obj{other.p_obj}, p_cb{other.p_cb}
    {
        other.p_obj = nullptr;
        other.p_cb = nullptr;
    }
...
int main()
{
    ...
    int *x = &p;
    auto deleter = [&](int *) {};
    getcracked::shared_ptr<int, decltype(deleter)> intptr(x, deleter);
    std::cout << intptr.use_count() << " ";
    getcracked::shared_ptr<int, decltype(deleter)> anotherone(std::move(intptr));
